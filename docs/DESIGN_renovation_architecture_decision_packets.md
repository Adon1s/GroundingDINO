...
| Cutover — observation (C5) OUTCOME | window **CLOSED one day early** (day 6 of 7); no rollback trigger fired: 0 ontology/resolver/writer failures, all published artifacts carry complete root v5 + v4, gate clean, guard denials failed closed. 14 attempts / 10 artifacts over the 2 active days (08-22, 08-25; both stopped at exactly 5 listings on the Sol ceiling); 4 failures = 2× `SolDailyBudgetExceeded` (one on v4's 2f) + 2× lone-surrogate `UnicodeEncodeError` in postprocessing (engine-agnostic). Freeze on estimate behaviour/prompts/catalog lifted 2026-08-27 | 2026-08-27 | Steven | full record: `docs/DECISION_renovation_architecture_session9_cutover_20260821.md` §10; carry-forwards: unicode postprocessing bug (unowned), 2f = binding Sol consumer (the S8 forcing function), thumbnail thread starts 2026-08-29+ |
...
| Quality wave 1 — final scope | **Complete: QP3 only.** QP6, QP5, and QP4 were not implemented and are deferred outside wave 1 | 2026-08-27 | Steven | focused review found QP5's stored reject-with-split authority ambiguous and QP4's per-surrogate child partition unrepresentable with current merged work-item ids; implementation/replay record: `docs/RESULT_quality_wave1_qp3_20260827.md` |
