...
## Approved investigatory emphasis

Steven clarified that locating useful observations lost downstream is the primary objective; launch readiness is secondary. Preserve observation-level lineage and the earliest supported loss stage, distinguish policy exclusions and representation elsewhere, and rank actionable tuning hypotheses. Do not treat a final Terra rejection as proof that Terra introduced the error. The Session A recommendations were explicitly approved in this task; the corroboration gate stays off, Qwen 2d stays at 0.1, both reliability fixes proceed, and the six cases remain documented misses without a new paid experiment. No new task or session is authorized.
