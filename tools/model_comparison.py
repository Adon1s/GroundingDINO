...
Usage:
    ...
    python tools/model_comparison.py --property redfin_126224899 --skip-2d-isolated
"""
...
async def _run_2d_batch(
    ...
    for row in forward:
        ...
        candidates = _retrieve_candidates_for_obs(
            retriever=retriever,
            description=desc,
            kind=kind,
            scene_group=scene_group,
...
_JUDGE_SYSTEM_2A = """\
You are an expert real estate photo analyst evaluating two vision models head-to-head.

You will see an image and two freeform observation lists from Gemma and Qwen.
...
_JUDGE_SYSTEM_2B = """\
You are an expert evaluating two models' ability to decompose freeform notes into structured JSON.
...
_JUDGE_SYSTEM_2C = """\
You are an expert evaluating two models' ability to label observations by taxonomy.
...
You are an expert evaluating two models' ability to resolve observations to catalog item IDs.
...
You are an expert evaluating two models on the COUPLED 2c+2d chain — each model's own 2c labels
fed its own 2d candidate pick. This measures real-world chain quality.
...
You are an expert real estate photo analyst performing an END-TO-END evaluation of two local
VLM pipelines (Gemma and Qwen). You will see the photo AND each model's FINAL forward-set —
the real issues (defects + upgrade candidates) each model's chain produced after 2a→2b→2c.
...
You MUST also list:
- gemma_unique_good  — items Gemma caught that Qwen missed AND are truly present
- qwen_unique_good   — same for Qwen
- gemma_hallucinations — items Gemma claimed that are NOT visible / NOT defensible
- qwen_hallucinations  — same for Qwen
...
{
  "skill": "2a",
  "winner": "gemma" | "qwen" | "tie",
  "scores": {
    "gemma": {"accuracy": 0-5, "completeness": 0-5, "hallucination_penalty": 0-5, "specificity": 0-5},
    "qwen":  {"accuracy": 0-5, "completeness": 0-5, "hallucination_penalty": 0-5, "specificity": 0-5}
  },
  "gemma_unique_good": ["..."],
  "qwen_unique_good": ["..."],
  "gemma_hallucinations": ["..."],
  "qwen_hallucinations": ["..."],
  "missed_by_both": ["..."],
  "rationale": "..."
}"""
        ...
        return """\
{
  ...
  "winner": "gemma" | "qwen" | "tie",
  "scores": {
    "gemma": {"faithfulness_to_input": 0-5, "schema_adherence": 0-5, "decomposition_quality": 0-5, "no_speculation": 0-5},
    "qwen":  {"faithfulness_to_input": 0-5, "schema_adherence": 0-5, "decomposition_quality": 0-5, "no_speculation": 0-5}
  },
  "rationale": "..."
}"""
    if skill == "2c":
        return """\
{
  ...
  "winner": "gemma" | "qwen" | "tie",
  "scores": {
    "gemma": {"label_correctness": 0-5, "forward_set_purity": 0-5, "no_dimension_leakage": 0-5},
    "qwen":  {"label_correctness": 0-5, "forward_set_purity": 0-5, "no_dimension_leakage": 0-5}
  },
  "rationale": "..."
}"""
        ...
        return """\
{
  ...
  "winner": "gemma" | "qwen" | "tie",
  "scores": {
    "gemma": {"pick_correctness": 0-5, "no_hallucinated_ids": 0-5, "null_when_appropriate": 0-5},
    "qwen":  {"pick_correctness": 0-5, "no_hallucinated_ids": 0-5, "null_when_appropriate": 0-5}
  },
  "per_observation": [
    {
      "observation": "...",
      "judge_correct_id": "..." | null,
      "gemma_correct": true | false,
        ...
        return """\
{
  ...
  "winner": "gemma" | "qwen" | "tie",
  "scores": {
    "gemma": {"end_to_end_resolution_quality": 0-5, "label_kind_consistency": 0-5, "final_pick_correctness": 0-5},
    "qwen":  {"end_to_end_resolution_quality": 0-5, "label_kind_consistency": 0-5, "final_pick_correctness": 0-5}
  },
  "per_observation": [
    {
      "observation": "...",
...
def _build_user_prompt_2a(record: ImageRecord) -> str:
    return (
        f"## Scene: {record.scene}\n\n"
        f"## Gemma freeform observations:\n{record.gemma.pass_2a or '(empty)'}\n\n"
        f"## Qwen freeform observations:\n{record.qwen.pass_2a or '(empty)'}\n\n"
        f"## Your task:\nScore each model. Return JSON matching:\n{_verdict_schema_for('2a')}"
    )


def _build_user_prompt_2b(record: ImageRecord) -> str:
    return (
        f"## Shared input freeform (from GPT-5.4, identical for both models):\n"
        f"{record.fixture.pass_2a or '(empty)'}\n\n"
        f"## Gemma structured observations:\n{json.dumps(record.gemma.pass_2b, indent=2)}\n\n"
        f"## Qwen structured observations:\n{json.dumps(record.qwen.pass_2b, indent=2)}\n\n"
        f"## Your task:\nScore each model. Return JSON matching:\n{_verdict_schema_for('2b')}"
    )


def _build_user_prompt_2c(record: ImageRecord) -> str:
    return (
        f"## Shared input observations (from GPT-5.4):\n{json.dumps(record.fixture.pass_2b, indent=2)}\n\n"
        f"## Gemma labeled output:\n{json.dumps(record.gemma.pass_2c, indent=2)}\n\n"
        f"## Qwen labeled output:\n{json.dumps(record.qwen.pass_2c, indent=2)}\n\n"
...
def _build_user_prompt_2d_isolated(record: ImageRecord) -> str:
    gemma_rows = [_twod_row_to_prompt_dict(r) for r in record.gemma.pass_2d_isolated]
    qwen_rows = [_twod_row_to_prompt_dict(r) for r in record.qwen.pass_2d_isolated]
    return (
        f"## Shared input (GPT-5.4 labels, identical for both models):\n"
        f"{json.dumps(record.fixture.pass_2c, indent=2)}\n\n"
        f"## Gemma 2d decisions:\n{json.dumps(gemma_rows, indent=2)}\n\n"
        f"## Qwen 2d decisions:\n{json.dumps(qwen_rows, indent=2)}\n\n"
        f"## Your task:\nScore each model and, per observation, provide judge_correct_id. "
        f"Return JSON matching:\n{_verdict_schema_for('2d_isolated')}"
    )


def _build_user_prompt_2d_coupled(record: ImageRecord) -> str:
    gemma_rows = [_twod_row_to_prompt_dict(r) for r in record.gemma.pass_2c_2d_coupled]
    qwen_rows = [_twod_row_to_prompt_dict(r) for r in record.qwen.pass_2c_2d_coupled]
    return (
        f"## Shared source observations (from GPT-5.4 2b):\n"
        f"{json.dumps(record.fixture.pass_2b, indent=2)}\n\n"
        f"## Gemma coupled 2c+2d rows (each model used its OWN 2c):\n"
        f"{json.dumps(gemma_rows, indent=2)}\n\n"
        f"## Qwen coupled 2c+2d rows:\n{json.dumps(qwen_rows, indent=2)}\n\n"
...
def _build_user_prompt_comprehensive(record: ImageRecord) -> str:
    """End-to-end: image + both models' final forward-sets (after their 2a→2b→2c chains)."""
    gemma_forward = _forward_set(record.gemma.pass_2c)
    qwen_forward = _forward_set(record.qwen.pass_2c)
    return (
        f"## Scene: {record.scene} (group: {record.scene_group})\n\n"
        f"## Gemma's final forward-set ({len(gemma_forward)} items):\n"
        f"{json.dumps(gemma_forward, indent=2)}\n\n"
        f"## Qwen's final forward-set ({len(qwen_forward)} items):\n"
        f"{json.dumps(qwen_forward, indent=2)}\n\n"
...
async def run_judge_for_skill(
    ...
    """Run a single per-skill judge call. Returns the parsed verdict dict or None on failure."""
    if skill not in _SKILL_PROMPT_BUILDERS:
        raise ValueError(f"Unknown skill: {skill}")

    system_prompt, prompt_builder, needs_image = _SKILL_PROMPT_BUILDERS[skill]
    user_prompt = prompt_builder(record)
    ...
    try:
        ...
        logger.warning(f"Judge returned unparseable response for {record.photo_key}/{skill}: {e}")
        return None
...
def backfill_2d_judge_into_rows(
    ...
    The judge returns per_observation entries; we match by observation text and copy
    judge_correct_id, then compute rank in the candidate list and whether it's present.
    """
    if not verdict:
        return
    ...
    for row in rows:
        entry = by_desc.get(row.observation.strip())
        if not entry:
            continue
...
def build_recommendation(aggregate: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Per-pass config: ignore 2c/2d split and defer to coupled winner if they disagree
    per_pass_2c = best_2c if best_2c != "tie" else best_2cd_coupled
    per_pass_2d = best_2d_iso if best_2d_iso != "tie" else best_2cd_coupled
    if per_pass_2c != per_pass_2d and per_pass_2c != "tie" and per_pass_2d != "tie":
        # Never send 2c and 2d to different models in per-pass config
        per_pass_2c = best_2cd_coupled if best_2cd_coupled != "tie" else per_pass_2c
        per_pass_2d = per_pass_2c
        rationale_note = (
            f"best_2c ({best_2c}) and best_2d_isolated ({best_2d_iso}) split; "
            f"per_pass_config defers both to best_2c+2d_coupled ({best_2cd_coupled})."
        )
    else:
        rationale_note = ""
    ...
    suggested_per_pass = {
        ...
        "2c": per_pass_2c if per_pass_2c != "tie" else "gemma",
        "2d": per_pass_2d if per_pass_2d != "tie" else "gemma",
    }
    ...
    rationale_parts = [
        ...
        "models for different passes) and suggested_config_coupled_2cd (treats 2c+2d as atomic, "
        "simpler to deploy) based on operational preference.",
    ]
    if rationale_note:
        rationale_parts.append(rationale_note)
...
def _phase_complete(record: ImageRecord, phase: str) -> bool:
    ...
    if phase == "gemma":
        return record.gemma.pass_2a is not None or bool(record.gemma.pass_2b) or bool(record.gemma.pass_2c)
    if phase == "qwen":
        return record.qwen.pass_2a is not None or bool(record.qwen.pass_2b) or bool(record.qwen.pass_2c)
    if phase == "judge":
        return all(record.judge.get(s) is not None for s in SKILLS)
...
async def _run_phase_local(
    ...
    async def _worker(i: int, img: Dict[str, Any]) -> None:
        async with sem:
            key = f"{img['property_id']}/{img['photo_key']}"
            rec = records[key]
            if _phase_complete(rec, phase_name):
                logger.info(f"  [{i}/{n}] [cached] {key}")
                return
    ...
    async def _worker(i: int, key: str, rec: ImageRecord) -> None:
        async with sem:
            ...
            for skill in active_skills:
                ...
                try:
                    verdict = await run_judge_for_skill(skill, rec, vlm_client, judge_config)
                except Exception as e:
                    err_str = str(e)
                    if any(kw in err_str.lower() for kw in ("quota", "billing", "resourceexhausted")):
...
async def async_main(args: argparse.Namespace) -> None:
    ...
    _phase_needed = any(not _phase_complete(records[k], "gemma") for k in records)
    ...
    _phase_needed = any(not _phase_complete(records[k], "qwen") for k in records)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Max concurrent images for local LM Studio phases (A and B). Default: 1 "
             "(safe: LM Studio serializes internally and can KV-cache-corrupt under load). "
             "Bump to 2-3 only if your LM Studio setup is known-stable.",
    )
